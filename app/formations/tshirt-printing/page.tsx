...
export default function FormationTshirtPrinting() {
  ...
  const handleSubscribe = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch("/api/formations/subscribe", {
        ...
        body: JSON.stringify({ email, nom, formation: "Tshirt Printing" }),
      })
      if (res.ok) {
        setSuccess(true)
        setEmail("")
      ...
      <section style={{minHeight:"60vh",padding:"6rem 1.25rem",paddingTop:"12rem",textAlign:"center",background:"linear-gradient(135deg, rgba(212,168,83,0.1) 0%, transparent 100%)"}}>
        <div style={{maxWidth:"56rem",margin:"0 auto"}}>
          ...
          <p className="animate-fade-in-up" style={{fontSize:"1.125rem",color:"rgba(255,255,255,0.7)",lineHeight:1.8,marginBottom:"2rem",fontWeight:700,animationDelay:"0.3s"}}>
            Clients illimites. Profits garantis.
          </p>
          ...
          <div className="animate-fade-in-up" style={{display:"grid",gridTemplateColumns:"repeat(3, 1fr)",gap:"1rem",margin:"3rem 0",animationDelay:"0.4s"}}>
            <div style={{background:"rgba(212,168,83,0.1)",borderRadius:"0.5rem",padding:"1rem"}}>
              <p style={{fontSize:"0.875rem",fontWeight:900}}>Setup Production</p>
            </div>
            <div style={{background:"rgba(212,168,83,0.1)",borderRadius:"0.5rem",padding:"1rem"}}>
              <p style={{fontSize:"0.875rem",fontWeight:900}}>Haute Marge</p>
            </div>
            <div style={{background:"rgba(212,168,83,0.1)",borderRadius:"0.5rem",padding:"1rem"}}>
              <p style={{fontSize:"0.875rem",fontWeight:900}}>Scale Facile</p>
            </div>
          </div>
        </div>
      </section>
      ...
      <section style={{padding:"6rem 1.25rem",borderTop:"1px solid rgba(255,255,255,0.1)"}}>
        <div style={{maxWidth:"1400px",margin:"0 auto"}}>
          ...
          <div className="animate-fade-in-up" style={{marginBottom:"6rem"}}>
            <h2 style={{fontSize:"2rem",fontWeight:900,marginBottom:"2.5rem",textAlign:"center"}}>Maitrise les 3 Techniques</h2>
            
            <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit, minmax(250px, 1fr))",gap:"2rem"}}>
              {[
                {title:"DTF (Direct to Garment)",desc:"Couleurs illimitees, petites quantites"},
                {title:"Broderie",desc:"Premium look, haute marge"},
                {title:"Serigraphie",desc:"Volume, couleurs limitees, top ROI"},
                {title:"Clients B2B",desc:"Uniformes, corporates, recurrents"},
                {title:"Pricing Strategy",desc:"3x margin minimum"},
                {title:"Automation",desc:"Fulfillment, shipping, invoicing"}
              ].map((item, i) => (
          ...
          <div className="animate-fade-in-up" style={{background:"linear-gradient(135deg, rgba(212,168,83,0.1) 0%, transparent 100%)",borderRadius:"1.5rem",padding:"3rem",marginBottom:"6rem",border:"1px solid rgba(212,168,83,0.2)",textAlign:"center"}}>
            <h2 style={{fontSize:"2rem",fontWeight:900,marginBottom:"2rem"}}>Nos Producteurs Gagnent</h2>
            <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit, minmax(200px, 1fr))",gap:"2rem"}}>
              {[
                {amount:"$5K",period:"par mois",desc:"Producteurs modestes"},
                {amount:"$25K",period:"par mois",desc:"Createurs avec audience"},
                {amount:"$100K+",period:"par mois",desc:"Nos top performers"}
              ].map((item, i) => (
                <div key={i} style={{background:"#0A0A0B",border:"1px solid rgba(212,168,83,0.3)",borderRadius:"1rem",padding:"2rem"}}>
                  <p style={{fontSize:"2rem",fontWeight:900,color:"#D4A853",marginBottom:"0.5rem"}}>{item.amount}</p>
                  <p style={{fontWeight:700,marginBottom:"0.5rem"}}>{item.period}</p>
                  <p style={{color:"rgba(255,255,255,0.6)",fontSize:"0.875rem",fontWeight:700}}>{item.desc}</p>
                </div>
              ))}
            </div>
          ...
          <div className="animate-fade-in-up" style={{background:"#141416",borderRadius:"1.5rem",padding:"3rem",textAlign:"center",border:"2px solid #D4A853",marginBottom:"6rem"}}>
            <div style={{fontSize:"1.25rem",fontWeight:900,color:"#D4A853",marginBottom:"0.75rem"}}>30 PLACES RESTANTES</div>
            <p style={{fontSize:"1.125rem",fontWeight:900,marginBottom:"1rem"}}>Formation ferme dans 5 jours</p>
            <p style={{color:"rgba(255,255,255,0.7)",fontWeight:700}}>Prochain groupe: mois prochain</p>
          </div>
          ...
          <div className="animate-fade-in-up" style={{maxWidth:"28rem",margin:"0 auto",background:"linear-gradient(135deg, rgba(212,168,83,0.15) 0%, rgba(212,168,83,0.05) 100%)",border:"1px solid rgba(212,168,83,0.3)",borderRadius:"1.5rem",padding:"3rem"}}>
            <h2 style={{fontSize:"1.75rem",fontWeight:900,marginBottom:"1rem",textAlign:"center"}}>Rejoindre Maintenant</h2>
            ...
            {!success?(
              ...
              <div style={{background:"rgba(212,168,83,0.1)",border:"1px solid #D4A853",borderRadius:"0.5rem",padding:"2rem",textAlign:"center"}}>
                <p style={{color:"#D4A853",fontWeight:900,fontSize:"1.25rem",marginBottom:"0.5rem"}}>✓ Succès!</p>
                <p style={{color:"rgba(255,255,255,0.7)",fontWeight:700}}>Verifie ton email pour commencer.</p>
              </div>
            )}

            <p style={{marginTop:"1.5rem",fontSize:"0.75rem",color:"rgba(255,255,255,0.5)",fontWeight:700,textAlign:"center"}}>Pas de carte · Acces immediat</p>
          </div>
        </div>
      </section>
